...
map_kernel_window(uint64_t sbi_pt)
{
    ...
    *(((uint64_t *) &l1pt[511])) = sbi_pt;
    ...
    //write_csr(sptbr, addrFromPPtr(l1pt) >> RISCV_4K_PageBits);
 ...
 vm_rights_t vmRights, vm_attributes_t attr, pde_t *pd)
{
    ...
    case RISCV_2M_Page: {
        ...
        if (* ((uint64_t *) lu_lvl2_ret.ptSlot) != NULL) {
            current_syscall_error.type =
                seL4_DeleteFirst;
            current_syscall_error.failedLookupWasSource =
                false;
    ...
    case RISCV_1G_Page: {
        ...
        if (* ((uint64_t *) lu_pd_ret.pdSlot) != NULL) {
            current_syscall_error.type =
                seL4_DeleteFirst;
            current_syscall_error.failedLookupWasSource =
                false;
                           ...
                           word_t *buffer)
{
    ...
    case RISCVPageMap: {
        ...
        if (unlikely(cap_frame_cap_get_capFMappedAddress(cap))) {
            current_syscall_error.type =
        ...
        if (frameSize == RISCV_4K_Page) {
            ...
            if (unlikely(lu_lvl2pt_ret.status == EXCEPTION_NONE && *((uint64_t *)lu_lvl2pt_ret.ptSlot) != NULL)) {
                userError("RISCVPageMap: The page is already mapped %p", vaddr);
                current_syscall_error.type =
                    seL4_DeleteFirst;
                current_syscall_error.invalidCapNumber = 0;
            ...
            if (unlikely(lu_pd_ret.status == EXCEPTION_NONE && *((uint64_t *)lu_pd_ret.pdSlot) != NULL)) {
                userError("RISCVPageMap: The page is already mapped %p", vaddr);
                current_syscall_error.type =
                    seL4_DeleteFirst;
                current_syscall_error.invalidCapNumber = 0;
